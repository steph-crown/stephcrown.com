import { FC } from 'react'
import { ExperienceType } from 'Types'

type Props = {
  experience: ExperienceType
}

const ExperienceCard: FC<Props> = ({ experience }) => {
  const { role, company, startDate, endDate, technologies } = experience
  const dateRange = `${startDate} - ${endDate}`

  return (
    <article className='bg-portfolio-card rounded-none p-8 h-full flex flex-col' aria-label={`${role} at ${company}`}>
      <div className='flex flex-col sm:flex-row gap-2 sm:gap-6 md:gap-10'>
        <div className='flex-shrink-0 sm:w-24 md:w-28'>
          <p className='text-portfolio-muted text-sm'>{dateRange}</p>
        </div>

        <div className='flex-1 min-w-0'>
          <h3 className='text-portfolio-fg text-base mb-2'>{role}</h3>

          <p className='text-portfolio-muted text-sm leading-[142%]'>{company}</p>

          {technologies && <p className='text-[#BEBEBE] text-sm mt-14'>{technologies}</p>}
        </div>
      </div>
    </article>
  )
}

export default ExperienceCard
